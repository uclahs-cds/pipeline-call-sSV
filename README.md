...
| map_qual | yes | integer | Minimum paired-end (PE) mapping quality (MAPQ) for Delly.|
| min_clique_size | yes | integer | Minimum number of supporting PE or split-read (SR) alignments required for a clique to be identified as a structural variant by Delly. Adjust this parameter to control the sensitivity and specificity of Delly variant calling. |
| mad_cutoff | yes | integer | Insert size cutoff, median+s*MAD (deletions only) for Delly.|
